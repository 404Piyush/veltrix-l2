#!/bin/bash
set -euo pipefail

ROOT_DIR=$(cd "$(dirname "$0")/.." && pwd)
ENV_FILE=${ENV_FILE:-}

L1_RPC=${L1_RPC:-}
L2_RPC=${L2_RPC:-http://localhost:9546}
ROLLUP_RPC=${ROLLUP_RPC:-http://localhost:7546}
L1_DEPLOYMENTS=${L1_DEPLOYMENTS:-"$ROOT_DIR/configs/l1-deployments.sepolia.json"}

DEPOSIT_AMOUNT_WEI=${DEPOSIT_AMOUNT_WEI:-500000000000000}
WITHDRAW_AMOUNT_WEI=${WITHDRAW_AMOUNT_WEI:-100000000000000}
BRIDGE_GAS_LIMIT=${BRIDGE_GAS_LIMIT:-100000}
FINALIZE_WITHDRAWAL=${FINALIZE_WITHDRAWAL:-0}
WITHDRAW_USE_UNLOCKED=${WITHDRAW_USE_UNLOCKED:-0}
DEPOSIT_WAIT_ATTEMPTS=${DEPOSIT_WAIT_ATTEMPTS:-360}

L2_TO_L1_MESSAGE_PASSER=0x4200000000000000000000000000000000000016
TRANSACTION_DEPOSITED_TOPIC=0xb3813568d9991fc951961fcb4c784893574240a28925604d09fc577c55bb7c32
MESSAGE_PASSED_TOPIC=0x02a52367d10742d8032712c1bb8e0144ff1ec5ffda1ed7d70bb05a2744955054

load_env_file() {
    local path="$1"
    if [ -f "$path" ]; then
        # shellcheck disable=SC1090
        set -a
        . "$path"
        set +a
    fi
}

if [ -n "$ENV_FILE" ]; then
    load_env_file "$ENV_FILE"
else
    load_env_file "$ROOT_DIR/.env"
    load_env_file "$ROOT_DIR/ops/.env"
fi

require_command() {
    if ! command -v "$1" >/dev/null 2>&1; then
        echo "Error: $1 is required."
        exit 1
    fi
}

require_env() {
    local name="$1"
    if [ -z "${!name:-}" ]; then
        echo "Error: required env var '$name' is not set."
        exit 1
    fi
}

rpc_ready() {
    local rpc=$1
    curl -fsS -X POST "$rpc" \
        -H 'content-type: application/json' \
        --data '{"jsonrpc":"2.0","method":"eth_blockNumber","params":[],"id":1}' >/dev/null
}

wait_for_rpc() {
    local name=$1
    local rpc=$2

    for attempt in {1..45}; do
        if rpc_ready "$rpc"; then
            return
        fi
        sleep 2
    done

    echo "Error: $name RPC is not ready at $rpc."
    exit 1
}

receipt_has_topic() {
    local receipt=$1
    local topic=$2
    echo "$receipt" | jq -e --arg topic "$topic" '.logs[]?.topics[0] == $topic' >/dev/null
}

wait_for_balance_increase() {
    local address=$1
    local before=$2

    for attempt in $(seq 1 "$DEPOSIT_WAIT_ATTEMPTS"); do
        local after
        after=$(cast balance --rpc-url "$L2_RPC" "$address")
        if [ "$after" != "$before" ]; then
            echo "$after"
            return
        fi
        sleep 2
    done

    echo "Error: L2 balance for $address did not change after deposit." >&2
    exit 1
}

require_command cast
require_command curl
require_command jq

if [ -n "${PRIVATE_KEY:-}" ] && [ -z "${DEPLOYER_PRIVATE_KEY:-}" ]; then
    DEPLOYER_PRIVATE_KEY="$PRIVATE_KEY"
fi

L1_RPC=${L1_RPC:-${L1_RUNTIME_RPC_URL:-${L1_RPC_URL:-}}}

require_env L1_RPC
require_env DEPLOYER_PRIVATE_KEY

DEPOSIT_PRIVATE_KEY=${DEPOSIT_PRIVATE_KEY:-$DEPLOYER_PRIVATE_KEY}
WITHDRAWER_PRIVATE_KEY=${WITHDRAWER_PRIVATE_KEY:-$DEPOSIT_PRIVATE_KEY}
WITHDRAW_FROM=${WITHDRAW_FROM:-}

wait_for_rpc L1 "$L1_RPC"
wait_for_rpc L2 "$L2_RPC"
wait_for_rpc Rollup "$ROLLUP_RPC"

if [ ! -f "$L1_DEPLOYMENTS" ]; then
    echo "Error: missing $L1_DEPLOYMENTS."
    exit 1
fi

OPTIMISM_PORTAL_PROXY=$(jq -r '.OptimismPortalProxy // empty' "$L1_DEPLOYMENTS")
if [ -z "$OPTIMISM_PORTAL_PROXY" ]; then
    echo "Error: $L1_DEPLOYMENTS is missing OptimismPortalProxy."
    exit 1
fi

DEPOSIT_FROM=$(cast wallet address --private-key "$DEPOSIT_PRIVATE_KEY")
DEPOSIT_TO=${DEPOSIT_TO:-$DEPOSIT_FROM}
WITHDRAW_TO=${WITHDRAW_TO:-$DEPOSIT_FROM}

portal_code=$(cast code --rpc-url "$L1_RPC" "$OPTIMISM_PORTAL_PROXY")
if [ "$portal_code" = "0x" ]; then
    echo "Error: no OptimismPortal code at $OPTIMISM_PORTAL_PROXY on L1."
    exit 1
fi

echo "Testing Sepolia-backed L1 -> L2 deposit..."
deposit_before=$(cast balance --rpc-url "$L2_RPC" "$DEPOSIT_TO")
deposit_receipt=$(cast send \
    --rpc-url "$L1_RPC" \
    --private-key "$DEPOSIT_PRIVATE_KEY" \
    "$OPTIMISM_PORTAL_PROXY" \
    'depositTransaction(address,uint256,uint64,bool,bytes)' \
    "$DEPOSIT_TO" \
    "$DEPOSIT_AMOUNT_WEI" \
    "$BRIDGE_GAS_LIMIT" \
    false \
    0x \
    --value "$DEPOSIT_AMOUNT_WEI" \
    --json)

if [ "$(echo "$deposit_receipt" | jq -r '.status')" != "0x1" ]; then
    echo "Error: deposit transaction failed."
    echo "$deposit_receipt" | jq .
    exit 1
fi

if ! receipt_has_topic "$deposit_receipt" "$TRANSACTION_DEPOSITED_TOPIC"; then
    echo "Error: deposit receipt did not include TransactionDeposited."
    echo "$deposit_receipt" | jq .
    exit 1
fi

deposit_after=$(wait_for_balance_increase "$DEPOSIT_TO" "$deposit_before")
deposit_tx=$(echo "$deposit_receipt" | jq -r '.transactionHash')
echo "Deposit ok: $deposit_tx"
echo "L2 balance changed: $deposit_before -> $deposit_after"

echo "Testing Sepolia-backed L2 -> L1 withdrawal initiation..."
if [ -z "$WITHDRAW_FROM" ]; then
    WITHDRAW_FROM=$(cast wallet address --private-key "$WITHDRAWER_PRIVATE_KEY")
fi

if [ "$WITHDRAW_USE_UNLOCKED" = "1" ]; then
    withdraw_receipt=$(cast send \
        --rpc-url "$L2_RPC" \
        --unlocked \
        --from "$WITHDRAW_FROM" \
        "$L2_TO_L1_MESSAGE_PASSER" \
        'initiateWithdrawal(address,uint256,bytes)' \
        "$WITHDRAW_TO" \
        "$BRIDGE_GAS_LIMIT" \
        0x \
        --value "$WITHDRAW_AMOUNT_WEI" \
        --json)
else
    withdraw_receipt=$(cast send \
        --rpc-url "$L2_RPC" \
        --private-key "$WITHDRAWER_PRIVATE_KEY" \
        "$L2_TO_L1_MESSAGE_PASSER" \
        'initiateWithdrawal(address,uint256,bytes)' \
        "$WITHDRAW_TO" \
        "$BRIDGE_GAS_LIMIT" \
        0x \
        --value "$WITHDRAW_AMOUNT_WEI" \
        --json)
fi

if [ "$(echo "$withdraw_receipt" | jq -r '.status')" != "0x1" ]; then
    echo "Error: withdrawal initiation failed."
    echo "$withdraw_receipt" | jq .
    exit 1
fi

if ! receipt_has_topic "$withdraw_receipt" "$MESSAGE_PASSED_TOPIC"; then
    echo "Error: withdrawal receipt did not include MessagePassed."
    echo "$withdraw_receipt" | jq .
    exit 1
fi

withdraw_tx=$(echo "$withdraw_receipt" | jq -r '.transactionHash')
withdrawal_data=$(echo "$withdraw_receipt" | jq -r ".logs[] | select(.topics[0] == \"$MESSAGE_PASSED_TOPIC\") | .data" | head -1)
withdrawal_hash="0x$(echo "${withdrawal_data#0x}" | cut -c193-256)"
echo "Withdrawal initiated: $withdraw_tx"
echo "Withdrawal hash: $withdrawal_hash"

if [ "$FINALIZE_WITHDRAWAL" = "1" ]; then
    echo "Sepolia finalization requested; reusing finalize helper."
    L1_RPC="$L1_RPC" L2_RPC="$L2_RPC" L1_DEPLOYMENTS="$L1_DEPLOYMENTS" ./ops/finalize-sepolia-withdrawal.sh "$withdraw_tx"
fi

echo "Sepolia-backed bridge smoke test passed."
